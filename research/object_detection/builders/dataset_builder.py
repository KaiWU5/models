...
def read_dataset_folder(file_read_func, input_files, config):
  """Reads a dataset, and handles repetition and shuffling.
  ...
  """
  # Shard, shuffle, and read files.
  with open(os.path.join(input_files), "r+") as f:
...
def build(input_reader_config, batch_size=None, transform_input_data_fn=None):
  ...
  if input_reader_config.WhichOneof('input_reader') == 'external_input_reader':
    config = input_reader_config.external_input_reader
    if not config.input_file:
      raise ValueError('At least one input dir(images and labels) must be specified in '
                       '`input_reader_config`.')
    ...
    def process_with_folder(value, folder):
      """Sets up tf graph that decodes, transforms and pads input data."""
      processed_tensors = decoder.decode_folder(value, folder)
      if transform_input_data_fn is not None:
        processed_tensors = transform_input_data_fn(processed_tensors)
      return processed_tensors
    process_fn = functools.partial(process_with_folder, folder=config.dataset_dir[:])

    dataset = read_dataset_folder(
        functools.partial(tf.data.TFRecordDataset, buffer_size=8 * 1000 * 1000),
        config.input_file[:], input_reader_config)
